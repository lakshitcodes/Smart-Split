...
export const spendingInsights = inngest.createFunction(
    ...
    async ({ step }) => {
        ...
        for (const user of users) {
            ...
            const expenses = await step.run(`Fetch last month expenses for ${user.name}`, async () => {
                convex.query(api.inngest.getUserMonthlyExpenses, { userId: user._id });
            });

            if (!expenses || expenses.length === 0) {
                console.log(`No expenses for user ${user.name} (${user.email}), skipping email.`);
                continue;
            }

            /* c. JSON blob for prompt */
            ...
            const html = `<!DOCTYPE html>
                ...
                <body style="margin:0;padding:0;background:#f9fafb;font-family:system-ui,-apple-system,BlinkMacSystemFont,'Segoe UI',Roboto,sans-serif;color:#111827;">
                    ...
                    <div style="text-align:center;margin-top:24px;">
                        <a href="https://smartsplit.app" style="display:inline-block;padding:12px 24px;background:linear-gradient(90deg,#059669,#14b8a6);color:#fff;font-weight:600;text-decoration:none;border-radius:9999px;">
                    ...
                    <!-- Footer -->
                    <div style="text-align:center;margin:24px 0;font-size:13px;color:#6b7280;">
